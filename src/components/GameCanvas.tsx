...
import type { TerrainType, Bug, Enemy, ResourceNode } from '@/game/types';
...
export default function GameCanvas() {
  ...
  const draw = useCallback(() => {
    ...
    for (let gy = 0; gy < GRID_H; gy++) {
      for (let gx = 0; gx < GRID_W; gx++) {
        ...
        if (cell.type === 'forest') {
          ...
          const ty = py + CELL / 2;
          ctx.beginPath();
          ctx.arc(tx - 2, ty - 1, 3, 0, Math.PI * 2);
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(nestPos.x, nestPos.y, 22, 0, Math.PI * 2);
    ctx.stroke();
    ctx.strokeStyle = 'rgba(168, 85, 247, 0.4)';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.arc(nestPos.x, nestPos.y, 30, 0, Math.PI * 2);
    ctx.stroke();

    ctx.fillStyle = '#c084fc';
    ctx.beginPath();
    ctx.arc(nestPos.x, nestPos.y, 10, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#fde68a';
    ctx.beginPath();
    ctx.arc(nestPos.x, nestPos.y, 4, 0, Math.PI * 2);
    ctx.fill();

    for (const r of resources) {
      drawResource(ctx, r, tick);
    }
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      if (t - lastStepRef.current >= interval) {
        for (let i = 0; i < Math.min(3, Math.ceil(spd)); i++) {
          step();
        }
      ...
      {state.levelComplete && (
        <div className="absolute inset-0 bg-black/70 flex items-center justify-center backdrop-blur-sm z-10">
          <div className="text-center p-8 rounded-2xl border-2 border-emerald-500/50 bg-emerald-500/5"
            ...
            <div className="text-emerald-200/70 mb-6">{state.levelObjective} — 达成</div>
            <button
...
function drawBug(ctx: CanvasRenderingContext2D, bug: Bug, tick: number) {
  ...
  ctx.beginPath();
  ctx.arc(size * 0.8, -size * 0.35, size * 0.25, 0, Math.PI * 2);
  ctx.arc(size * 0.8, size * 0.35, size * 0.25, 0, Math.PI * 2);
  ctx.fill();
...
function drawResource(ctx: CanvasRenderingContext2D, r: ResourceNode, tick: number) {
  ...
  const size = (5 + (r.amount / r.maxAmount) * 4) * pulse;
